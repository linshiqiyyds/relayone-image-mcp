...
type ProviderRequest = { method: "GET" | "POST"; body?: string; headers?: Record<string, string> };
type ToolContent =
...
async function saveImageResponse(
  ...
  const response = result as { data: Array<Record<string, unknown>>; output_format?: unknown };
...
server.tool("prepare_image_request", "Preview the outgoing JSON without contacting the provider.", imageRequestSchema, async (input) => {
  try { const provider = await selectedProvider(input.provider); return { content: [{ type: "text", text: asText({ endpoint: `${baseUrl(provider)}${provider.imageGenerationsPath}`, body: imageRequestBody(input, provider) }) }] }; }
  catch (error) { return { isError: true, content: [{ type: "text", text: error instanceof Error ? error.message : String(error) }] }; }
});
